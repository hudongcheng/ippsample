...
static void bgra_to_gray(unsigned char *row, size_t num_pixels);
static void bgra_to_rgb(unsigned char *row, size_t num_pixels);
...
/*
* 'bgra_to_rgb()' - Pack BGRX scanlines into RGB scanlines.
*
* This routine is suitable only for 8 bit BGRX data packed into RGB bytes.
*/

static void
    bgra_to_rgb(unsigned char *row,		/* I - Row of pixels to pack */
size_t        num_pixels)	/* I - Number of pixels in row */
{
    unsigned char R, G, B;
    int i = 0, y = 0;
    unsigned char *dest = row;
    size_t imgSize = num_pixels * 4;
    for ( ; i < imgSize; ) {
        B = row[i];
        G = row[i+1];
        R = row[i+2];
...
/*
* 'bgra_to_gray()' - Pack BGRX scanlines into Gray scanlines.
*
* This routine is suitable only for 8 bit BGRX data packed into Gray bytes.
*/

static void
bgra_to_gray(unsigned char *row,		/* I - Row of pixels to pack */
    size_t num_pixels)	            /* I - Number of pixels in row */
{
    unsigned char L, R, G, B;
    
    unsigned char *gray = row;
    size_t imgSize = num_pixels * 4;
    for (int i = 0; i < imgSize; ) {
        B = row[i];
        G = row[i+1];
        R = row[i+2];
        L = (unsigned char)(R * 299/1000 + G * 587/1000 + B * 114/1000);
        gray[i/4] = L;
        i += 4;
    }
}
...
void dump_raw(const unsigned char* buffer, size_t size, const char* filename){
...
void             *ctx)		/* I - Write context */
{
    FPDF_DOCUMENT	doc;	    /* Document to print */
    FPDF_PAGE		pdf_page;	/* Page in PDF file */
    FPDF_BITMAP		bitmap;	    /* Bitmap for page */
/*    fz_colorspace		*cs;		/* Quartz color space */
    xform_raster_t	ras;		/* Raster info */
    size_t		max_raster;	/* Maximum raster memory to use */
    const char		*max_raster_env;/* IPPTRANSFORM_MAX_RASTER env var */
...
/*    back_transform;	/* Transform for back side */
    ...
    */
    ...
    doc = FPDF_LoadDocument(filename, NULL);
    if (!doc) {
      fprintf(stderr, "ERROR: Unable to load '%s': %l\n", filename, FPDF_GetLastError());
      return (1);
    }
    ...
    */
    ...
    if ((page_ranges = cupsGetOption("page-ranges", num_options, options)) != NULL)
    {
        ...
        pages = (unsigned)FPDF_GetPageCount(doc);
        if (first > pages)
        {
            fputs("ERROR: \"page-ranges\" value does not include any pages to print in the document.\n", stderr);
    ...
    {
        first = 1;
        last  = (unsigned)FPDF_GetPageCount(doc);
    }
...
/*    fz_set_aa_level(context, 0);
    ...
    */
    int width = ras.header.cupsWidth;
    int height = ras.header.cupsHeight;
    ...
    for (copy = 0; copy < ras.copies; copy ++)
    {
        for (page = 1; page <= pages; page ++)
        {
            ...
            pdf_page = FPDF_LoadPage(doc, (int)(page + first - 2));
            if(!pdf_page) {
                fprintf(stderr, "ERROR: Unable to load page: %l\n", FPDF_GetLastError());
            }
            /*
            fz_bound_page(context, pdf_page, &image_box);
...
*/
            ...
            //FPDF_RenderPageBitmapWithMatrix(bitmap, pdf_page, NULL, NULL, FPDF_ANNOT);
            FPDFBitmap_FillRect(bitmap, 0, 0, width, height, 0xFFFFFFFF);
            FPDF_RenderPageBitmap(bitmap, pdf_page, 0, 0, width, height, 0, FPDF_ANNOT);
            int stride = FPDFBitmap_GetStride(bitmap);
            unsigned char* buffer =
                (unsigned char*)FPDFBitmap_GetBuffer(bitmap);
            
            // dump raw
            dump_raw(buffer, width * height * 4, "color.raw");
            if (ras.header.cupsBitsPerPixel == 24) {
                /* rgba to gray */
                bgra_to_rgb(buffer, width * height);
                dump_raw(buffer, width*height*3, "rgb.raw");
            } else {
                /* rgba to rgb */
                bgra_to_gray(buffer, width * height);
                dump_raw(buffer, width*height, "gray.raw");
            }
            // dump gray
            //dump_raw(buffer, width*width, "gray.raw");
...
*/
                ...
                lineptr = buffer + y * ras.header.cupsBytesPerLine;
...
cups_option_t  *options)	/* I - Options */
{
    ...
    */
    ...
    fprintf(stderr, "LOG: pwg_media %s, width %d, length %d.\n ", pwg_media->pwg, pwg_media->width, pwg_media->length);
    ...
    */
    ...
    fprintf(stderr, "LOG: xdpi %d, ydpi %d.\n", xdpi, ydpi);
    ...
    */
    ...
    fprintf(stderr, "DEBUG: resolution = %d, cupsWidth = %d, cupsHeight = %d ,cupsBitsPerColor = %d, cupsBitsPerPixel= %d, cupsBytesPerLine = %d\n" 
        , ras->header.HWResolution[0], ras->header.cupsWidth, ras->header.cupsHeight, ras->header.cupsBitsPerColor, ras->header.cupsBitsPerPixel, ras->header.cupsBytesPerLine);
